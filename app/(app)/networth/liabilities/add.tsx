...
import type { LiabilityCategory, LiabilityType, CreateLiabilityRequest } from '@/types/models';
  ...
  const handleSaveLiability = async (liabilityData: CreateLiabilityRequest) => {
    ...
    try {
      ...
      if (success) {
        alert(
          'Debt Added',
          `"${liabilityData.name}" has been added to your liabilities successfully.`,
          [{ 
            text: 'OK', 
            style: 'default',
            onPress: () => router.back()
      ...
      alert('Error', 'Failed to add liability. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      {isLoading && (
        <View style={styles.loadingOverlay}>
          ...
          <Text style={styles.loadingText}>Adding debt...</Text>
        </View>
      )}
