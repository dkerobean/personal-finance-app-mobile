...
export default function AddAssetScreen() {
  ...
  const handleSaveAsset = async (assetData: CreateAssetRequest) => {
    if (!user?.id) {
      toast.error('Error', 'You must be logged in to add an asset');
      return;
    }

    setIsLoading(true);
    try {
      const success = await createAsset(user.id, assetData);
      if (success) {
        ...
        {isLoading ? (
          <View style={styles.loadingOverlay}>
            <View style={styles.loadingCard}>
              ...
              <Text style={styles.loadingText}>Saving asset...</Text>
            </View>
          </View>
        ) : null}
      </KeyboardAvoidingView>
