...
serve(async (req) => {
  ...
  try {
    ...
    if (!accountId) {
      return new Response(
        JSON.stringify({ error: { code: 'INVALID_REQUEST', message: 'Account ID is required' } }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
      ...
      for (const mtnTransaction of transactions) {
        try {
          ...
          const transactionData = {
            ...
            is_synced: true,
            auto_categorized: true, // Transaction was automatically categorized
