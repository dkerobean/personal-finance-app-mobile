...
const transformAsset = (asset: any): AssetType => ({
  ...
  units_held: asset.unitsHeld,
  unit_price: asset.unitPrice,
  last_valuation_date: asset.lastValuationDate,
...
export const assetsApi = {
  ...
  async create(userId: string, request: CreateAssetRequest): Promise<ApiResponse<AssetType>> {
    const result = await apiClient.post<any>('/assets', {
      ...
      valuationMethod: request.valuation_method,
      tickerSymbol: request.ticker_symbol,
      unitsHeld: request.units_held,
      unitPrice: request.unit_price,
      lastValuationDate: request.last_valuation_date,
