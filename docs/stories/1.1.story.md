...
- [x] Initialize mobile app and backend (AC: 1)
  - [x] Create Expo React Native project with TypeScript; adopt expo-router (tabs acceptable) [Source: `docs/architecture.md#Frontend Architecture`]
  - [x] Add and configure Supabase JS client for the mobile app [Source: `docs/architecture.md#Frontend Services Layer`]
  - [x] Create backend repo scaffold (Supabase project initialized, CLI ready) [Source: `docs/architecture.md#Development Workflow`]

- [x] Implement Registration flow (AC: 2, 3, 4)
  - [x] Screen `app/(auth)/register.tsx` with email and password inputs using Gluestack UI [Source: `docs/architecture.md#Frontend Architecture`]
  - [x] Client-side validation for email format and password strength [Source: `docs/architecture.md#Security and Performance`]
  - [x] Call Supabase Auth `signUp()` and handle result state [Source: `docs/architecture.md#Core Workflows`]
  - [x] Show confirmation that verification email was sent; navigate to code entry screen

- [x] Implement Email Verification (AC: 5, 6, 7)
  - [x] Screen `app/(auth)/verify.tsx` to accept verification code (OTP) [Source: `docs/architecture.md#Core Workflows`]
  - [x] Call Supabase Auth `verifyOtp()` on submission; on success, retrieve session/JWT [Source: `docs/architecture.md#Core Workflows`]
  - [x] Securely store JWT using Expo Secure Store [Source: `docs/architecture.md#Authentication Security`]
  - [x] Redirect authenticated user to `app/(app)/index.tsx` (Dashboard) [Source: `docs/architecture.md#Routing Architecture`]

- [x] Error handling and UX polish (AC: 8)
  - [x] Display clear error toasts/messages for invalid inputs, failed signups, and incorrect codes [Source: `docs/architecture.md#Frontend Error Handling`]
  - [x] Implement a minimal centralized API/Auth error handler to normalize error format [Source: `docs/architecture.md#Error Handling Strategy`]

- [x] Testing (all AC)
  - [x] Unit/component tests for registration and verification screens with Jest + RNTL [Source: `docs/architecture.md#Frontend Testing`]
  - [x] Integration test mocking Supabase Auth responses for signUp/verifyOtp happy and error paths [Source: `docs/architecture.md#Testing Strategy`]
  - [x] Optional E2E with Maestro for full happy-path registration + verification [Source: `docs/architecture.md#E2E Testing`]

- [x] Project structure and configuration
  - [x] Place shared Supabase client in `src/services/supabaseClient.ts` [Source: `docs/architecture.md#Frontend Services Layer`]
  - [x] Add basic auth store in `src/stores/authStore.ts` (Zustand) to track session [Source: `docs/architecture.md#State Management Architecture`]
  - [x] Ensure authenticated route group protection for `/app` using layout guard [Source: `docs/architecture.md#Routing Architecture`]
